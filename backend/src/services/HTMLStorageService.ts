  ...
  async saveProject(request: SaveProjectRequest): Promise<SavedProject> {
    ...
    try {
      ...
      const initialVersion: ProjectVersion = {
        ...
        html: request.pipelineResult.sections?.map(s => s.html).join('\n') || '',
      ...
      const project: SavedProject = {
        ...
        metadata: {
          ...
          sectionsCount: request.pipelineResult.sections?.length || 0,
          fieldsCount: request.pipelineResult.sections?.reduce((acc, s) => acc + (s.editableFields?.length || 0), 0) || 0,
          lastModified: now
        }
      };
  ...
  async updateProject(request: UpdateProjectRequest): Promise<SavedProject> {
    ...
    try {
      ...
      const version: ProjectVersion = {
        ...
        createdAt: now,
        changes: request.changes,
        html: request.html || project.pipelineResult.sections?.map(s => s.html).join('\n') || '',
