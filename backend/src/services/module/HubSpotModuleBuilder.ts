...
import { HubSpotValidationService, ValidationResult, HubSpotModule } from '../quality/HubSpotValidationService';
import { HubSpotPromptService, ModuleGenerationRequest, GeneratedModule } from '../deployment/HubSpotPromptService';
...
export class HubSpotModuleBuilder {
  ...
  private createManifest(moduleSlug: string, fields: DetectedField[]): ModuleManifest {
    return {
      label: moduleSlug.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
      module_slug: moduleSlug,
      version: '0.1.0',
      css_assets: [],
