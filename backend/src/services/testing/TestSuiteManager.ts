...
export class TestSuiteManager extends EventEmitter {
  ...
  private generateJestConfig(context: any): any {
    return {
      ...
      coverageDirectory: `${this.config.reportOutputDir}/coverage-${context.category}`,
  ...
  private async generateCoverageReport(executionId: string): Promise<CoverageReport | null> {
    try {
      const coverageDir = path.join(__dirname, `../../${this.config.reportOutputDir}/coverage`);
      const coverageFile = path.join(coverageDir, 'coverage-summary.json');

      if (fs.existsSync(coverageFile)) {
        const coverageData = JSON.parse(await fs.promises.readFile(coverageFile, 'utf8'));
