...
// Shared AI layout with Humanloop-style tabs (Maintenance)
// Routes under /maintenance/ai/[process]/: dashboard | editor | log-view
export default function MaintenanceAIProcessLayout({
  children,
  params,
}: {
  ...
  const base = `/maintenance/ai/${params.process}`;
              ...
              <nav className="p-2 space-y-1 max-h-[70vh] overflow-auto">
                {processes.map((p) => {
                  ...
                  return (
                    <Link
                      key={p.name}
                      href={`/maintenance/ai/${p.name}/editor`}
