...
// Logs page scaffold matching Humanloop-style table
// Toolbar: +Filter, Actions, View Sessions, Search, Inputs filter, Refresh
// Table: ID, Version, Inputs, Messages, Output, Rating, Correction, Environment, kebab
...
type RunRow = {
  ...
  correction?: string;
  env?: string;
  // optional raw payload to show in details
  raw?: any;
};
...
export default function LogsPage({ params }: { params: { process: string } }) {
  ...
  // const [drawerRow, setDrawerRow] = useState<RunRow | null>(null);
  ...
  const fetchRuns = useCallback(async () => {
    ...
    setError(null);
    try {
      // Backend provides: GET /api/admin/ai-prompts/processes/:process/runs?limit=...
      const limit = pageSize; // simple client-side paging for now
      ...
      if (Array.isArray(json?.data)) {
        ...
        // Apply client-side search + advanced filters
        ...
        const filteredByInputKey = inputKeyFilter
          ...
          : filteredByStatus;
        // Simple client-side pagination window
  ...
  useEffect(() => {
    fetchRuns();
  }, [fetchRuns, page, pageSize, search, inputsFilter, params.process, statusFilter, inputKeyFilter]);

  // Live refresh: subscribe to backend logs and count new items
  useEffect(() => {
    const onAnyLog = () => {
      // Increment counter, but avoid unbounded growth
      setNewItems((c) => (c < 999 ? c + 1 : c));
    };
          ...
          <tbody>
            ...
            {!loading && filtered.map((r) => (
              <tr
                ...
                onClick={() => router.push(`/ai/${params.process}/editor?runId=${encodeURIComponent(r.id)}`)}
              >
                <td className="p-3">
                  <input
                    type="checkbox"
                ...
                <td className="p-3 text-right">
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      router.push(`/ai/${params.process}/editor?runId=${encodeURIComponent(r.id)}`);
                    }}
                  >
                    ⋯
                  </button>
