...
describe('Modular Pipeline Integration Tests', () => {
  ...
  describe('Pipeline API Integration (function-based)', () => {
    test('should expose executePipeline function', () => {
      expect(typeof executePipelineFn).toBe('function');
    });
    ...
    test('should execute modular pipeline with all 5 phases', async () => {
      ...
      const result = await executePipelineFn(mockDesignFile);
    ...
    test('should handle pipeline errors gracefully with fallback', async () => {
      ...
      await expect(executePipelineFn(mockDesignFile)).rejects.toThrow('Pipeline execution failed');
